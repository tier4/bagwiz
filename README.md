...
`topic drop` / `topic keep` drop or keep selected topics in a bag (by literal
...
| [`bagwiz topic`](docs/commands/topic.md)       | Keep (`keep`) or drop (`drop`) topics in a ROS 2 rosbag, selected by literal name or `*` glob.             |
