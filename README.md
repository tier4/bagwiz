...
| [`bagwiz trim`](docs/commands/trim.md)         | Trim a ROS 2 rosbag to a time window given as offsets from the bag start (e.g. `--start 5s --end 90s`), or align it to the time span of selected topics (`--align`). Windows are evaluated on `header.stamp` by default (`--stamp recv` for record time). |
...
| [`bagwiz tf`](docs/commands/tf.md)             | Inspect and edit TF in a ROS 2 rosbag: frame tree, static-transform resolution, static-TF copy, and interactive transform walk.                                                                                                                           |
