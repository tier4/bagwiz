...
// Rewrites the calibration on a single CameraInfo topic. Non-target topics pass
// through verbatim. For the target topic, the original CDR is deserialized into
// a typed sensor_msgs/msg/CameraInfo (so the header, binning, and roi survive
// untouched), the calibration fields are overwritten from the YAML, and the
// message is re-serialized. The typesupport pointer is shared and read-only;
// transform() runs on the single producer thread, so the scratch typed message
// it builds is never shared across threads.
class CamInfoReplaceProcessor : public core::pipeline::Processor
{
public:
  CamInfoReplaceProcessor(
    std::string topic, const rosidl_message_type_support_t * typesupport,
    ...
    std::vector<std::byte> & out) const override
  {
    ...
    if (rmw_deserialize(&in_view, typesupport_, &msg) != RMW_RET_OK) {
      throw std::runtime_error(
        "failed to deserialize a message on '" + topic_ + "' as " + kCameraInfoType + ": " +
        take_rmw_error());
    }
    ...
    if (rmw_serialize(&msg, typesupport_, &serialized.get()) != RMW_RET_OK) {
      throw std::runtime_error(
        "failed to re-serialize a CameraInfo message on '" + topic_ + "': " + take_rmw_error());
    }
...
// type is unchanged), then stream-copy — rewriting the calibration on `topic`
// and forwarding everything else untouched. The writer factory is parameterised
// so the in-place path can hand in a tmp location.
int execute_pass(
  const std::filesystem::path & input_path, const std::string & topic,
  ...
  const std::function<std::unique_ptr<io::BagWriter>()> & open_writer)
{
  ...
  CamInfoReplaceProcessor processor(topic, typesupport, calibration, frame_id);
  ...
  BAGWIZ_LOG_INFO(
    kLogger,
    "cam-info replace: rewrote calibration on %" PRIu64 " message(s) of '%s', copied %" PRIu64
    " other message(s) verbatim.",
    counts.transformed, topic.c_str(), forwarded);
  if (counts.transformed == 0) {
    BAGWIZ_LOG_WARN(
      kLogger, "Topic '%s' carried no messages; the bag's calibration was left unchanged.",
      topic.c_str());
  }
  return 0;
}
...
{
  // 1. Inspect the bag and confirm <topic> exists and is a CameraInfo topic.
  std::unique_ptr<io::BagReader> reader;
  try {
  ...
  if (match->type != kCameraInfoType) {
    BAGWIZ_LOG_ERROR(
      kLogger, "Topic '%s' has type '%s', expected '%s'.", args.topic.c_str(), match->type.c_str(),
      kCameraInfoType);
    return 1;
  }
  ...
  if (args.output_path.has_value()) {
    ...
    return execute_pass(
      args.input_path, args.topic, intro.typesupport, calibration, args.frame_id, make_writer);
  }
  ...
  try {
    core::write_bag_inplace(args.input_path, [&](const std::filesystem::path & tmp) {
      pass_status = execute_pass(
        args.input_path, args.topic, intro.typesupport, calibration, args.frame_id,
