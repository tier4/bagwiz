...
public:
  ...
  int run() override
  {
    ...
    if (!topic_present_with_type(*reader, cloud_topic_, kPointCloud2Type)) {
      return 1;
    }
...
private:
    ...
    io::BagReader & reader, const std::string & topic, const char * expected_type)
  {
    ...
    if (info == nullptr) {
      BAGWIZ_LOG_ERROR(
        kLogger, "Topic '%s' is not present in %s", topic.c_str(), input_path_.c_str());
      return false;
    }
  ...
  bool peek_frames(std::string & cloud_frame, std::string & imu_frame)
  {
    ...
    filter.topics.push_back(imu_topic_);
    ...
    while ((cloud_frame.empty() || imu_frame.empty()) && reader->next(raw)) {
      if (cloud_frame.empty() && raw.topic->name == cloud_topic_) {
      ...
      } else if (imu_frame.empty() && raw.topic->name == imu_topic_) {
    ...
    if (cloud_frame.empty()) {
      BAGWIZ_LOG_ERROR(
        ...
        cloud_topic_.c_str(), std::to_string(cloud_fail).c_str());
      return false;
    }
    if (imu_frame.empty()) {
      BAGWIZ_LOG_ERROR(
        kLogger, "Could not read an Imu frame_id from '%s' (%s message(s) failed to parse)",
        imu_topic_.c_str(), std::to_string(imu_fail).c_str());
      return false;
    }
    return true;
  }
    ...
    std::int64_t & skipped, std::int64_t & imu_count)
  {
    ...
    try {
      while (reader.next(raw)) {
        if (raw.topic->name == cloud_topic_) {
          const auto parsed = core::pointcloud::parse_pointcloud2(raw.payload);
          if (!parsed.ok()) {
            ++skipped;
            continue;
          ...
          on_scan(*scan.scan);
          ++scans;
        } else if (!imu_topic_.empty() && raw.topic->name == imu_topic_) {
          const auto parsed = core::slam::parse_imu(raw.payload);
          if (!parsed.ok()) {
