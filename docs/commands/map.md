...
| `--cam <topic>...`       | Camera image topic(s) (`sensor_msgs/msg/Image` or `sensor_msgs/msg/CompressedImage`); list several after one flag and/or repeat the flag. After the global optimization, the map points are colorized by projecting them into each camera's images, and `map.pcd` gains an `rgb` field (points no image observed keep a neutral gray). A point observed by several cameras takes the color from the earliest listed topic that saw it. Intrinsics come from each camera's `CameraInfo` topic (auto-resolved from the image topic name; see `--cam-info`); each camera extrinsic is resolved from the bag's static TF (cloud ← camera optical frame), erroring if that chain is absent. Images are assumed raw (unrectified): the `CameraInfo` distortion model is applied during projection. |
...
| `-j, --threads N`        | Number of CPU threads for GLIM and trajectory endpoint fill (default: 4). The host's hardware concurrency is the effective maximum.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- **Camera colorization (`--cam`).** Runs after the global optimization: for
  ...
  color. The final color is the per-point average over that camera's
  observations; points no image ever observed keep a neutral gray. With
  several `--cam` topics, every camera runs its own accumulation over one
  shared bag pass and the results merge by `--cam` order: the earliest listed
  topic that observed a point provides its color, and later topics only fill
  points the earlier ones never saw. The image topics, `CameraInfo`s, and
...
# Colorize from two cameras; where their views overlap, the earliest listed
# topic wins. Listing both topics after one --cam and repeating the flag are
# equivalent.
bagwiz map slam drive.mcap /sensing/lidar/concatenated/pointcloud out/ \
  --cam /sensing/camera/camera8/image_raw/compressed \
