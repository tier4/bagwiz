...
| `--recover-init`         | Recover poses for the initialization window (requires `--imu`). GLIM's LiDAR-IMU odometry emits no pose until IMU init converges (~1 s), so `traj.tum` otherwise has no samples over its opening window. This buffers the pre-init IMU + scan stamps and, once the first frame's converged state is known, integrates the IMU backward from it to recover those early poses (re-anchored onto the optimized map). Affects `traj.tum`'s opening window only; the map is unaffected.                                                                                                                                                                                        |
...
- **Initialization window (`--recover-init`).** GLIM's LiDAR-IMU odometry emits
  no pose until its IMU initialization converges (~1 s), so `traj.tum` normally
  starts a beat late. With `--recover-init`, the pre-init IMU and scan stamps are
  buffered; once the first estimated frame's converged state (pose, velocity,
  biases) is known, the IMU is integrated backward from it to recover per-scan
  poses for that window, re-anchored onto the optimized map. LiDAR-IMU only
  (requires `--imu`); affects `traj.tum`'s opening window only.
