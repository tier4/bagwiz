...
| `--pcd <pcd_topic>`            | `sensor_msgs/msg/PointCloud2` topic to run SLAM on.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `-o`, `--output <output_root>` | Output directory. Receives `traj.tum` and `map.pcd`. Created if absent.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `--color <topic>...`           | Camera image topic(s) (`sensor_msgs/msg/Image` or `sensor_msgs/msg/CompressedImage`) to colorize the map from; list several after one flag and/or repeat the flag. The accepted types are exactly those `walk`'s image preview and `generate video` decode (the shared image decoder). After the global optimization, the map points are colorized by projecting them into each camera's images, and `map.pcd` gains an `rgb` field. Each point's color is a robust weighted average over its observations, computed in linear light (the sRGB pixel values are decoded before any averaging and re-encoded on export); a point observed by several cameras gets a weighted blend of them (gain-aligned to the first listed topic), and points no image observed inherit the nearest observed neighbor's color unless `--no-color-propagate` is passed (then they keep a neutral gray). Intrinsics come from each camera's `CameraInfo` topic (auto-resolved from the image topic name; see `--cam-info`); each camera extrinsic is resolved from the bag's static TF (cloud ← camera optical frame), erroring if that chain is absent. Images are assumed raw (unrectified): the `CameraInfo` distortion model is applied during projection. A topic listed more than once is an error.                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `--cam <topic>...`             | Camera image topic(s) (`sensor_msgs/msg/Image` or `sensor_msgs/msg/CompressedImage`) to use as SLAM visual constraints; list several after one flag and/or repeat the flag. The accepted types are exactly those `walk`'s image preview and `generate video` decode (the shared image decoder — the same set `--color` accepts). Each camera's KLT feature tracks become co-visibility constraints between submap poses in the global optimization, curbing drift; tracks confined to a single submap constrain nothing and are dropped, so yield grows with more, smaller submaps (see `--submap-keyframes`). Observations are anchored by interpolating the camera's intra-submap pose at the image's own stamp, so a `--cam` camera need not be synchronized with the LiDAR; triangulated landmarks are rejected on excess reprojection error (which is what catches moving objects and drifted tracks) and separately cross-checked against the submap's LiDAR cloud (which rejects points with no LiDAR backing at all, e.g. sky or reflections). Tracking runs on one worker thread per camera during the SLAM pass, off the critical path; zero usable constraints only warns. Independent of `--color` (a topic may serve both roles — its `CameraInfo` and extrinsic are then resolved once and shared, but a `--cam`-only topic is never colorized). Intrinsics come from each camera's `CameraInfo` topic (see `--cam-info`); each camera extrinsic is resolved from the bag's static TF (cloud ← camera optical frame), erroring if that chain is absent. Images are assumed raw (unrectified) and global-shutter, with header stamps trusted as capture time. A topic listed more than once is an error. |
| `--visual-max-features <N>`    | Target live KLT feature-track count per `--cam` camera (default `200`; must be positive; requires `--cam`). More features give denser co-visibility — stronger, more numerous constraints — at more CPU per tracked frame; fewer trades constraint strength for tracking speed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `--color-min-dist <m>`         | Keyframe gate for `--color` colorization (requires `--color`; default `0` = use every image): an image is colorized only when the interpolated pose has moved at least this many meters — or rotated at least 10° — since the image that opened the current gate interval on that topic. Consecutive frames are near-duplicates on vehicle data (a platform stopped at a light contributes hundreds of frames of identical information), so thinning cuts colorize time roughly in proportion to the frames dropped with no visible quality loss; `1`–`2` m is a good starting point. Thinned frames skip the decode too (unless `--color-keyframe-blur` is on, which must decode every candidate to score it). The per-camera colorize summary logs the thinned count. Deterministic.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `--color-keyframe-blur`        | Refine the `--color-min-dist` gate by sharpness (requires `--color-min-dist`): instead of keeping the FIRST image of each gate interval, every candidate inside the interval is decoded and scored with a whole-image sharpness metric (mean Sobel gradient magnitude) and the sharpest is kept, so motion-blurred frames are dropped rather than merely down-weighted (the per-observation sharpness weight only discounts blurry pixels after the frame was already rasterized). Costs a decode per candidate image; the projection/sweep work is still saved. Deterministic.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
With `--cam`, the run also logs a summary of the visual constraints applied (no
file output — `traj.tum`/`map.pcd` are unaffected by whether any were found):
a per-camera `Visual tracking: fed <N> image(s) from '<topic>' (<M> failed to
decode or track)` line, a `visual constraints: <N> factors from <M> tracks
over <K> submaps (dropped: ...)` breakdown from the constraint-building pass,
and a closing `Applied <N> visual constraint(s) from <M> track(s) across <K>
--cam camera(s)` line (a WARN instead when zero constraints were applied). See
"Camera constraints (`--cam`)" below for what drives those counts.
...
- **LiDAR-only vs LiDAR-IMU.** Without `--imu`, odometry runs from the cloud
  ...
  whenever the GPU backend is active — see `--backend`) fuses the IMU, and the
...
- **Camera colorization (`--color`).** Runs after the global optimization: for
  ...
  distortion model. With `--color-min-dist`, images are first thinned to
  keyframes — kept only when the pose moved (or rotated) enough since the
  image that opened the current interval on that topic, and with `--color-keyframe-blur` each gate
  ...
  map. All color averaging and every gain ratio run in linear light — the
  sRGB pixel values are decoded before the arithmetic and re-encoded on
  export — so blending observations of different exposures estimates the
  surface's radiance instead of the systematically darker gamma-space mean. With several `--color` topics, every
...
- **Camera constraints (`--cam`).** Runs during the SLAM pass itself, not
  after: each `--cam` camera gets its own KLT feature tracker fed on one
  dedicated worker thread per camera, so the per-frame decode and tracking
  stay off the bag-read/odometry critical path. Once every worker has drained
  (at map finalization), the buffered per-track observations are turned into
  co-visibility constraints between submap poses in the global optimization
  graph, curbing drift the same way `--gnss`'s priors do. Each observation is
  anchored to the submap whose LiDAR keyframe span covers its stamp by
  interpolating that submap's intra-submap LiDAR trajectory (SE(3), slerp
  rotation / lerp translation) at the image's own capture stamp, so a `--cam`
  camera's images need not be synchronized with the LiDAR scans. A track that
  never leaves a single submap constrains nothing between poses and is dropped
  before it costs a triangulation — so constraint yield grows with the number
  of submap boundaries a bag crosses: a lower `--submap-keyframes` (more,
  smaller submaps) or a longer/faster-moving recording both raise the fraction
  of tracks that survive to a factor. A surviving track's landmark is
  triangulated at the submaps' pose estimates; the triangulation itself
  rejects a track whose worst reprojection error is too large, which is what
  catches moving objects (traffic or pedestrians the submap's LiDAR never
  captured there) and drifted tracks. The triangulated point is then
  separately cross-checked against those submaps' LiDAR point clouds (a
  voxel-occupancy gate: it must fall in an observed voxel in at least one
  submap it was seen from), which rejects points with no LiDAR backing at
  all — sky, reflections, distant background — that would otherwise tie
  submaps together through geometry the LiDAR map never saw. Applying zero
  constraints (too little parallax, every track confined to one submap, a
  reprojection-error rejection, or the LiDAR gate rejecting every landmark)
  only warns; the run still completes with a map unconstrained
  by the cameras, exactly like an unproductive `--gnss`. Cameras are assumed
  global-shutter, with the header stamp trusted as the instant of capture — no
  rolling-shutter or camera↔LiDAR time-offset modeling (a documented
  limitation tracked in #376, the same assumption `--color` colorization makes
  per #366). `--cam` is independent of `--color`: the two topic lists may be
  disjoint, identical, or overlapping; a topic in both roles has its
  `CameraInfo`/extrinsic resolved once and shared, but a `--cam`-only topic is
  never colorized and a `--color`-only topic never contributes a visual
  constraint.
...
- **Color fields.** The inspector's Field selector offers `x`/`y`/`z` (plus
  ...
  A map with an `rgb` field (written by `map slam --color`) additionally offers
