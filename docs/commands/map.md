...
> **Build.** `map slam` links the GLIM stack (compiled with `-DBAGWIZ_WITH_SLAM=ON`)
> and is part of the default `pixi run build` on **humble**/**jazzy**:
>
> ```bash
> pixi run -e humble build   # or: jazzy
> ```
>
> The first `build` builds the GLIM dependency stack (GTSAM, gtsam_points, GLIM)
> into `install/<distro>/glim-deps` — a slow one-time step (tens of minutes) — then
> compiles bagwiz with SLAM enabled. Later builds reuse the cached deps and are
> fast. **lyrical** is the exception: it cannot pin the Eigen version GLIM needs, so
> `pixi run -e lyrical build` produces a non-SLAM bagwiz that does not expose
> `bagwiz map slam` or `bagwiz map filter`.
>
> **GPU fast path.** For the optional CUDA backend (`--backend gpu`), build in a
> `*-gpu` environment — the CUDA toolkit is pixi-managed (conda-forge), so no
> system CUDA install is needed, only an NVIDIA driver + GPU:
>
> ```bash
> pixi run -e humble-gpu build-gpu   # or: jazzy-gpu
> ```
>
> The `humble-gpu` environment is the distro plus the conda CUDA toolkit (12.8:
> nvcc, cudart, cusolver). This builds a CUDA GLIM stack (sm_86) into
> `install/humble-gpu/glim-deps-cuda` and compiles bagwiz with
> `-DBAGWIZ_WITH_SLAM_CUDA=ON` into `install/humble-gpu`. CUDA comes entirely from
> `$CONDA_PREFIX` (no `/usr/local/cuda`). Run it with
> `pixi run -e humble-gpu run-gpu -- map slam …`, or put it on your PATH with
> `pixi run -e humble-gpu install-gpu`. The CPU environments stay CUDA-free, and
...
| Flag                     | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--imu <topic>`          | `sensor_msgs/msg/Imu` topic. Switches odometry to LiDAR-IMU (GLIM's `OdometryEstimationCPU`, or `OdometryEstimationGPU` when combined with `--backend gpu`). The LiDAR←IMU extrinsic is resolved from the bag's static TF using the cloud and IMU header `frame_id`s.                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `--backend <mode>`       | SLAM backend: `auto` (default), `cpu`, or `gpu`. `auto` uses the CUDA GPU backend when this binary was built with `-DBAGWIZ_WITH_SLAM_CUDA` (`pixi run build-gpu`) **and** a CUDA device is visible, otherwise it falls back to CPU. `gpu` forces the GPU backend (errors on a non-CUDA build or with no device). `cpu` forces the CPU backend (the reproducibility-guaranteed path). GPU backend = GPU LiDAR-IMU odometry with `--imu` (CT odometry without it, as GLIM has no GPU LiDAR-only backend), GPU VGICP registration in sub/global mapping, and GPU export-map voxelization. **The GPU backend is outside the CPU reproducibility guarantee.** |
...
- **LiDAR-only vs LiDAR-IMU.** Without `--imu`, odometry runs from the cloud
  ...
  under `--backend gpu`) fuses the IMU, and the
