...
| `--threads N`            | Number of CPU threads for GLIM's preprocessor and odometry. `0` (default) keeps GLIM's built-in defaults (preprocess=2, odometry=4). Larger values speed up CPU-bound work on multi-core machines.                                                                                                                   |
