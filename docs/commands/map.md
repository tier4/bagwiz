...
>
> ```bash
> pixi run -e humble-gpu build-full-cuda   # or: jazzy-gpu
> ```
>
...
> `pixi run -e humble-gpu run-cuda -- map slam …`, or put it on your PATH with
> `pixi run -e humble-gpu install-cuda`. The CPU environments stay CUDA-free, and
