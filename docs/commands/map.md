...
>
> **GPU fast path.** For the optional CUDA backend (`--gpu`), build with:
>
> ```bash
> pixi run -e humble build-slam-gpu   # or: jazzy
> ```
>
> This builds a CUDA GLIM stack (sm_86) into `install/<distro>/glim-deps-cuda`
> (reusing the CPU prefix's GTSAM, so run `build-slam`/`build-glim` first) and
> compiles bagwiz into a separate `install/<distro>-gpu` base with
> `-DBAGWIZ_WITH_SLAM_CUDA=ON`. It needs CUDA 12.8 and an NVIDIA GPU; the CPU
> build, prefix, and its reproducibility guarantee are untouched.
...
| Flag                     | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--imu <topic>`          | `sensor_msgs/msg/Imu` topic. Switches odometry to LiDAR-IMU (GLIM's `OdometryEstimationCPU`, or `OdometryEstimationGPU` when combined with `--gpu`). The LiDAR←IMU extrinsic is resolved from the bag's static TF using the cloud and IMU header `frame_id`s.                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `--gpu`                  | Shorthand for `--backend gpu` (mutually exclusive with `--backend`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- **LiDAR-only vs LiDAR-IMU.** Without `--imu`, odometry runs from the cloud
  ...
  under `--gpu`) fuses the IMU, and the
