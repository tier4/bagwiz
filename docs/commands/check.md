...
```text
bagwiz check broken <input> [--rm] [--deep]
```
...
| Name    | Description                                                                                                                                                                  |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `input` | A single rosbag (`*.mcap` / `*.db3` / `*.db3.zstd`, or a rosbag2 directory) or a directory to scan. A directory is walked recursively and every rosbag within it is checked. |
...
bagwiz check broken ~/data/rosbags/

# Check a single bag.
bagwiz check broken capture.mcap

# Delete every broken bag found, no prompt.
bagwiz check broken ~/data/rosbags/ --rm

# Thorough scan that reads every message.
bagwiz check broken ~/data/rosbags/ --deep

# Feed the broken-bag list into another tool.
bagwiz check broken ~/data/rosbags/ > broken.txt
```
