...
| `BAGWIZ_READ_THREADS`    | integer 0–16 (default 8, capped at the host's core count); unparsable values fall back to the default with a warning          | Worker count for the parallel read paths: per-chunk decompression on mcap, per-slice scanning on db3. `0` or `1` falls back to the single-threaded read on both — the debugging escape hatch. Emitted messages are identical either way.                                                                                                                                       | commands that read mcap or db3 bags                                                                 | `bagwiz_io/src/io/read_tuning.hpp`                |
