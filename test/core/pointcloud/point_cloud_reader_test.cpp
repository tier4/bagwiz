  ...
  const std::vector<std::tuple<float, float, float, std::uint8_t>> & points)
{
  ...
  for (const auto & p : points) {
    append_bytes(data, f32_bytes(std::get<0>(p)));
    append_bytes(data, f32_bytes(std::get<1>(p)));
    append_bytes(data, f32_bytes(std::get<2>(p)));
...
TEST(PointCloudReaderTest, ExtractsXYZIntensityUint8)
{
  ...
  EXPECT_FLOAT_EQ(read_float_field(result.view, 0, *result.view.x_offset), 1.0f);
  EXPECT_FLOAT_EQ(read_float_field(result.view, 0, *result.view.y_offset), 2.0f);
  EXPECT_FLOAT_EQ(read_float_field(result.view, 1, *result.view.z_offset), 6.0f);
...
TEST(PointCloudReaderTest, ExtractsXYZIntensityUint16)
{
  ...
  const auto append_point = [&](float x, float y, float z, std::uint16_t intensity) {
    ...
    data.push_back(static_cast<std::byte>(intensity & 0xFFU));
    data.push_back(static_cast<std::byte>((intensity >> 8) & 0xFFU));
    data.push_back(std::byte{0});
    data.push_back(std::byte{0});
  };
